		...
		<div class="bodytext_area box_inner">
			...
			<table class="bbsListTbl" summary="번호,제목,조회수,작성일 등을 제공하는 표">
				...
				<tbody>
					<c:forEach items="${boardList}" var="boardVO" varStatus="status">
						<tr>
							<td>${boardVO.bno}</td>
							<td><a href="/board/view?bno=${boardVO.bno}&page=
								${pageVO.page}">${boardVO.title}</a></td>
			...
			<div class="pagination">
				...
				<c:forEach begin="${pageVO.startPage}" end="${pageVO.endPage}" var="idx">
					...
					<span class='pagenum <c:out value="${idx==pageVO.page?'currentpage':''}"/>'>${idx}</span></a>
				</c:forEach>
				<c:if test="${pageVO.next}">
					...
					<img src="/resources/home/img/btn_nextpage.png" alt="다음 페이지로 이동"></a>
				</c:if>
			</div>
			<!-- //pagination -->
		<p class="btn_line">
