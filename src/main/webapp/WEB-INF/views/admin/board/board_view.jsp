					...
					<form role="form" action="/admin/board/delete" method = "post">
						<div class="row">
								...
								<div class="form-group">
									...
									<select name="bod_type" class="form-control" required>
										<option value="">게시판 선택</option>
											...
											<option value="${boardTypeMenu.bod_type}"<c:out value="${(boardVO_bod_type eq boardTypeMenu.bod_type)?('selected'):('')}"/>>${boardTypeMenu.bod_name}</option>
										</c:forEach>
									</select>
								</div>
							</div>
							...
							<div class="col-sm-12">
								...
								<div class="form-group">
									<label>Content</label>
									<br><textarea name="content"class="form-control" rows="3"
							...
							<form role="form">
								<div class="row">
									<div class="col-sm-12">
										<!-- text input -->
										<div class="form-group">
											<label>Writer</label> <input id="replyerInput" type="text" class="form-control"
												placeholder="USER ID">
										</div>
									</div>
									<div class="col-sm-12">
									...
									<a href="javascript:;" id="insertApplyBtn" type="submit" class="btn btn-info">ADD REPLY</a>
								</div>
...
<!-- .//Content Wrapper. Contains page content -->
<!-- 댓글 관련 자바스크립트 시작 -->
<!-- 댓글리스트 반복문용 jQuery라이브러리 == jstl의 forEach와 같은 역할 -->
...
<!-- 댓글에 사용되는 변수,함수 초기화 / 댓글 템플릿(빵틀) 출력 -->
<script>
//댓글 변수+함수 초기화
var bno = ${boardVO.bno};
